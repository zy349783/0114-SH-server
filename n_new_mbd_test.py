...
def mbdGene(stockData):
    ...
    try:
        ...
        data['open'] = mdLog[mdLog['open'] != 0]['open'].iloc[0]
...
thisDate = datetime.date(2020, 1, 3)
while thisDate <= datetime.date(2020, 1, 3):
    thisDate_str = str(thisDate).replace('-', '')
    db = DB("192.168.10.178", 'com_md_eq_cn', 'zhenyuy', 'bnONBrzSMGoE')

    mdOrderLog = db.read('md_order', start_date=thisDate_str, end_date=thisDate_str, symbol=[2002192])
    
    if mdOrderLog is None:
        thisDate = thisDate + datetime.timedelta(days=1)
        continue
    print(thisDate)
    mdTradeLog = db.read('md_trade', start_date=thisDate_str, end_date=thisDate_str, symbol=[2002192])
