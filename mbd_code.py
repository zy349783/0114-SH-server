...
if __name__ == '__main__':
    ...
    db = DB("192.168.10.178", 'com_md_eq_cn', 'zhenyuy', 'bnONBrzSMGoE')
    # start date
    thisDate = datetime.date(2020, 1, 1)
    while thisDate <= datetime.date(2020, 7, 31):
        ...
        print(thisDate)
        os.mkdir('/mnt/e/mbd_data_new/' + thisDate_str)
        ...
        re = re.rename(columns={'date': "count1"})
        re = re.sort_values(by='count1', ascending=False)
        sl = list(re['skey'].values.astype('int64'))
