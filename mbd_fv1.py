...
class SimMktSnapshotAllNew():
    ...
    def updateMktInfo(self, clockAtArrival, exchangeTime, seqNum, isImprove, record=True):
        ...
        if record == True:
            ...
            self.bid_cancellation_qty.append(curBidCancellationQty)
            self.ask_cancellation_qty.append(curAskCancellationQty)
            self.aa.append(curBidCancellationAmount)
            self.ask_cancellation_amount.append(curAskCancellationAmount)
            # clear
            self.insertion_bid = defaultdict(int)
            self.insertion_ask = defaultdict(int)
...
#     # start date
#     thisDate = datetime.date(2020, 9, 24)
#     while thisDate <= datetime.date(2020, 9, 24):
...
#         mdOrderLog = db.read('md_order', start_date=thisDate_str, end_date=thisDate_str, symbol=[2300142])
#         if mdOrderLog is None:
#             thisDate = thisDate + datetime.timedelta(days=1)
#             continue

#         print(thisDate)
#         mdTradeLog = db.read('md_trade', start_date=thisDate_str, end_date=thisDate_str, symbol=[2300142])
#         mdLog = db.read('md_snapshot_l2', start_date=thisDate_str, end_date=thisDate_str, symbol=[2300142])

#         orders_data = {}
#         trades_data = {}
#         prev_data = {}
#         orders_data[2300142] = mdOrderLog
#         trades_data[2300142] = mdTradeLog
#         prev_data[2300142] = mdLog['prev_close'].iloc[0]
...
if __name__ == '__main__':
    ...
    while thisDate <= datetime.date(2019, 12, 31):
        ...
        # pool.map(g.run, test_list[20:60])
        # pool.close()
        # pool.join()
        # print(time.time() - start)
        
        li_st = np.arange(20, m-1400, 50)
