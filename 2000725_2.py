...
class SimMktSnapshotAllNew():
    ...
    def getAllInfo(self):
        ...
        targetCols = ['time', 'clockAtArrival', 'sequenceNo', 'StockID', 'cum_volume', 'cum_amount', 'close'] + \
                     bp_names[::-1] + ap_names + bq_names[::-1] + aq_names + bn_names[::-1] + an_names + btopK_names[
                                                                                                         ::-1] + atopK_names
        ...
        for col in ['time']:
            final_df[col] = (final_df[col] * 1000).astype('int64')
    ...
    def calcVwapInfo(self):
        ...
        for p in self.bid:
            bid_qty += self.bid[p]
            bid_amount += self.bid[p] * p
            bid_order_nums += len(self.bidnq[p].keys())
            bid_price_levels += 1
        ...
        for p in self.ask:
            ask_qty += self.ask[p]
            ask_amount += self.ask[p] * p
            ask_order_nums += len(self.asknq[p].keys())
            ask_price_levels += 1
...
if __name__ == '__main__':
    ...
    #start date
    thisDate = datetime.date(2018, 7, 30)
    while thisDate <= datetime.date(2018, 12, 31):
        ...
        test_list = list(re['skey'].values)
        m = len(test_list)
        print(m)
        test_list = test_list[0:20]
        try:
            test_list.remove(2000725)
            proc=19
        except:
