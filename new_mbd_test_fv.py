    ...
    try:
        ...
        for rowEntry in stockDataNP:
            if rowEntry.isAuction:
                ...
                # if rowEntry.ApplSeqNum == 435391:
                #     print(rowEntry)
                # if rowEntry.ApplSeqNum == 512805:
                #     print(rowEntry)
        ...
        #     del data
        data.to_pickle('/mnt/ShareWithServer/result/002049.pkl')
...
thisDate = datetime.date(2020, 8, 6)
while thisDate <= datetime.date(2020, 8, 6):
    thisDate_str = str(thisDate).replace('-', '')
    db = DB("192.168.10.178", 'com_md_eq_cn', 'zhenyuy', 'bnONBrzSMGoE')

    mdOrderLog = db.read('md_order', start_date=thisDate_str, end_date=thisDate_str, symbol=[2002043])
    
    if mdOrderLog is None:
        thisDate = thisDate + datetime.timedelta(days=1)
        continue
    print(thisDate)
    mdTradeLog = db.read('md_trade', start_date=thisDate_str, end_date=thisDate_str, symbol=[2002043])
