...
class generateMBD():
    ...
    def removeOrderByCancel(self, caa, exchgTm, ApplSeqNum, cancelQty, BidApplSeqNum, OfferApplSeqNum):
        ...
        if ((self.prevMktApplSeqNum > 0) & (cancelApplSeqNum == self.prevMktApplSeqNum) & (cancelOrderType == 1)) | ((cancelApplSeqNum != self.prevMktApplSeqNum) & (self.prevMktApplSeqNum > 0)):
            ApplSeqNum1 = self.Num[-2]
            hasUnfrozen = self.guessNBBO(caa, exchgTm, ApplSeqNum1, self.aggSide, -1, record=True)
    ...
    def guessNBBO(self, caa, exchgTm, ApplSeqNum, aggSide, bbo, record=True):
        ...
        self.matchTrades(fakeBidDict, fakeAskDict, self.trades, self.cum_volume, self.cum_amount, self.close, fakeBidnq1, fakeBidnq2, fakeAsknq1, fakeAsknq2, self.bid_qty, \
                         self.ask_qty, self.bid_amount, self.ask_amount, self.bid_price_levels, self.ask_price_levels, self.bid_order_nums, self.ask_order_nums, aggSide, Tag)
        ...
        if (self.skey >= 2300000) & (self.date >= 20200824):
            if aggSide == 1 and len(self.frozenBidDict) > 0:
                ...
                while len(self.frozenBidDict) > 0 and min(self.frozenBidDict.keys()) <= fakeMinBuyP:
                    ...
                    fakeBidDict, fakeAskDict, fake_trades, fake_cum_volume, fake_cum_amount, fake_close, fakeBid1p, fakeAsk1p, fakeBidnq1, fakeBidnq2, fakeAsknq1, fakeAsknq2, fakeBid_qty, fakeAsk_qty, fakeBid_amount, \
                    fakeAsk_amount, fakeBid_price_levels, fakeAsk_price_levels, fakeBid_order_nums, fakeAsk_order_nums, fakeHasTrade = \
                    self.matchTrades(fakeBidDict, fakeAskDict, fake_trades, fake_cum_volume, fake_cum_amount, fake_close, fakeBidnq1, fakeBidnq2, fakeAsknq1, fakeAsknq2, fakeBid_qty, fakeAsk_qty, fakeBid_amount, \
                    fakeAsk_amount, fakeBid_price_levels, fakeAsk_price_levels, fakeBid_order_nums, fakeAsk_order_nums, aggSide, Tag)
    ...
    def matchTrades(self, bidDict, askDict, trades, cum_volume, cum_amount, close, bidnq1, bidnq2, asknq1, asknq2, bid_qty, ask_qty, bid_amount, ask_amount, bid_price_levels, ask_price_levels, \
        ...
        if bid1p >= ask1p:
            while bid1p >= ask1p:
                ...
                if (tradeQty == bidQty) & (tradeQty != askQty):
                    m1 = 0
                    bidDict.pop(bid1p)
                    bidPLs = bidPLs[1:]
                    bid_price_levels -= 1
                ...
                elif (tradeQty == askQty) & (tradeQty != bidQty):
                    m1 = 0
                    askDict.pop(ask1p)
                    askPLs = askPLs[1:]
                    ask_price_levels -= 1
    ...
    def getSimMktInfo(self):
        ...
        mdData = pd.DataFrame({'caa': self.caaLs, 'time': self.exchgTmLs,  'ApplSeqNum': self.ApplSeqNumLs, 'cum_trades_cnt': self.cum_trades_cnt,
        ...
        targetCols = (['skey', 'date', 'time', 'caa', 'ApplSeqNum', 'cum_volume', 'cum_amount', 'cum_trades_cnt', 'close', 'bboImprove'] + \
