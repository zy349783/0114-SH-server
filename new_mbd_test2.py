...
thisDate = datetime.date(2020, 11, 6)
while thisDate <= datetime.date(2020, 11, 18):
    ...
    if mdOrderLog is None:
        ...
        continue
    print(thisDate)
    os.mkdir('/mnt/e/mbd_data1/' + thisDate_str)
    ...
    for i in range(len(li_st)):
        ...
        if i != len(li_st) - 1:
            ...
            m2 = m
        start = time.time()
        stockDataLs = [stockData for _, stockData in msgData[msgData['skey'].isin(test_list2[m1: m2])].groupby(['skey'])]
