...
class generateMBD():
    ...
    def removeOrderByAuctionTrade(self, caa, exchgTm, ApplSeqNum, price, qty, BidApplSeqNum, OfferApplSeqNum):
        ...
        cum_vol = 0
        for seqNo in self.asknq[askOrderPrice]:
            ...
            if cum_vol > qty:
                ...
                self.asknq[askOrderPrice].pop(seqNo)
                self.ask_qty -= useful_qty
                self.ask_amount -= useful_qty * askOrderPrice
                self.ask_order_nums -= 1
                break
    ...
    def removeOrderByTrade(self, caa, exchgTm, ApplSeqNum, price, qty, BidApplSeqNum, OfferApplSeqNum):
        ...
        if bidOrderQty == 0:
            self.orderDict.pop(BidApplSeqNum)
            self.bidnq[bidOrderPrice].pop(BidApplSeqNum)
            self.bid_order_nums -= 1
        else:
            self.orderDict[BidApplSeqNum] = (bidOrderPrice, bidOrderQty, bidOrderSide, bidOrderType, bidTag, 1)
        ...
        if askOrderQty == 0:
            self.orderDict.pop(OfferApplSeqNum)
            self.asknq[askOrderPrice].pop(OfferApplSeqNum)
            self.ask_order_nums -= 1           
        else:
            self.orderDict[OfferApplSeqNum] = (askOrderPrice, askOrderQty, askOrderSide, askOrderType, askTag, 1)
    ...
    def removeOrderByCancel(self, caa, exchgTm, ApplSeqNum, cancelQty, BidApplSeqNum, OfferApplSeqNum):
        ...
        if cancelApplSeqNum in self.frozenOrderDict:
            ...
            if cancelApplSeqNum in self.asknq[cancelPrice]:
                ...
                if self.asknq[cancelPrice][cancelApplSeqNum] == 0:
                    self.asknq[cancelPrice].pop(cancelApplSeqNum)
            elif cancelApplSeqNum in self.bidnq[cancelPrice]:
                self.bidnq[cancelPrice][cancelApplSeqNum] = np.int32(self.bidnq[cancelPrice][cancelApplSeqNum] - cancelQty)
                if self.bidnq[cancelPrice][cancelApplSeqNum] == 0:
                    self.bidnq[cancelPrice].pop(cancelApplSeqNum)      
    ...
    def guessNBBO(self, caa, exchgTm, ApplSeqNum, aggSide, record=True):
        ...
        time1 = time.time()
        fakeBidnq = deepcopy(self.bidnq)
        fakeAsknq = deepcopy(self.asknq)
        ...
        if record:
            ...
            curBidN = [len(list(fakeBidnq[i].keys())) for i in curBidP]
            ...
            curAskN = [len(list(fakeAsknq[i].keys())) for i in curAskP]
        ...
        if bid1p >= ask1p:
            while bid1p >= ask1p:
                ...
                if tradeQty == bidQty:
                    ...
                    bidPLs = bidPLs[1:]
                    bid_price_levels -= 1
                    bid_order_nums -= len(list(bidnq[bid1p].keys())) 
                else:
                    bidDict[bid1p] -= tradeQty
                    s_um = 0
                    pop_list = []
                    for seqNo in bidnq[bid1p]:
                ...
                if tradeQty == askQty:
                    ...
                    askPLs = askPLs[1:]
                    ask_price_levels -= 1
                    ask_order_nums -= len(list(asknq[ask1p].keys())) 
                else:
                    askDict[ask1p] -= tradeQty
                    s_um = 0
                    pop_list = []
                    for seqNo in asknq[ask1p]:
