...
class generateMBD():
    ...
    def insertAuctionOrder(self, caa, exchgTm, ApplSeqNum, side, orderType, price, qty):
        ...
        if side == 1:
            ...
            self.bidnq[price, ApplSeqNum] = np.int32(qty)
            self.bid_qty += qty
            ...
            self.asknq[price, ApplSeqNum] = np.int32(qty)
            self.ask_qty += qty
    ...
    def removeOrderByAuctionCancel(self, caa, exchgTm, ApplSeqNum, cancelQty, BidApplSeqNum, OfferApplSeqNum):
        ...
        if cancelApplSeqNum in [i[1] for i in self.asknq]:
            ...
            if self.asknq[cancelPrice, cancelApplSeqNum] == 0:
                self.asknq.pop((cancelPrice, cancelApplSeqNum))
        else:
            ...
            if self.bidnq[cancelPrice, cancelApplSeqNum] == 0:
                self.bidnq.pop((cancelPrice, cancelApplSeqNum))
    ...
    def removeOrderByAuctionTrade(self, caa, exchgTm, ApplSeqNum, price, qty, BidApplSeqNum, OfferApplSeqNum):
        ...
        cum_vol = 0
        for seqNo in [i[1] for i in self.bidnq if i[0] == bidOrderPrice]:
            ...
            if cum_vol > qty:
                ...
                self.bidnq.pop((bidOrderPrice, seqNo))
                self.bid_qty -= useful_qty
                self.bid_amount -= useful_qty * bidOrderPrice
                self.bid_order_nums -= 1
                break
        ...
        cum_vol = 0
        for seqNo in [i[1] for i in self.asknq if i[0] == askOrderPrice]:
            cum_vol += self.asknq[askOrderPrice, seqNo]
            if cum_vol > qty:
                useful_qty = (self.asknq[askOrderPrice, seqNo] - (cum_vol - qty))
                self.asknq[askOrderPrice, seqNo] = np.int32(cum_vol - qty)
                self.ask_qty -= useful_qty
                self.ask_amount -= useful_qty * askOrderPrice
                break
            elif cum_vol == qty:
                useful_qty = self.asknq[askOrderPrice, seqNo]
                self.asknq.pop((askOrderPrice, seqNo))
                self.ask_qty -= useful_qty
                self.ask_amount -= useful_qty * askOrderPrice
                self.ask_order_nums -= 1
                break
    ...
    def insertOrder(self, caa, exchgTm, ApplSeqNum, side, orderType, price, qty):
        ...
        if orderType == 2:
            ...
            elif ((side == 1 and price > self.maxLmtBuyP) or (side == 2 and price < self.minLmtSellP)) & (self.skey >= 2300000) & (self.date >= 20200824):
            ...
            if side == 1:
                ...
                self.bidnq[self.ask1p, ApplSeqNum] = np.int32(qty)
                ...
                self.asknq[self.bid1p, ApplSeqNum] = np.int32(qty)
                self.ask_qty += qty
            ...
            if side == 1:
                ...
                if self.bid1p in self.bidDict:
                    ...
                    self.bidnq[self.bid1p, ApplSeqNum] = np.int32(qty)
                    self.bid_qty += qty
                ...
                if self.ask1p in self.askDict:
                    ...
                    self.asknq[self.ask1p, ApplSeqNum] = np.int32(qty)
                    self.ask_qty += qty
    ...
    def removeOrderByTrade(self, caa, exchgTm, ApplSeqNum, price, qty, BidApplSeqNum, OfferApplSeqNum):
        ...
        if BidApplSeqNum in self.frozenOrderDict:
            self.bidDict, self.frozenBidDict, self.frozenOrderDict, self.frozenApplSeqNumDict, self.bid_price_levels, self.bid_order_nums, self.bidnq, self.bid_qty, self.bid_amount, \
                              self.ask_price_levels, self.ask_order_nums, self.asknq, self.ask_qty, self.ask_amount =\
            self.unfrozen(bidOrderPrice, 1, self.bidDict, self.frozenBidDict, self.frozenOrderDict, self.frozenApplSeqNumDict, self.bid_price_levels, self.bid_order_nums, self.bidnq, self.bid_qty, self.bid_amount, \
                              self.ask_price_levels, self.ask_order_nums, self.asknq, self.ask_qty, self.ask_amount)
            hasUnfrozen = 1
        
        if OfferApplSeqNum in self.frozenOrderDict:
            self.askDict, self.frozenAskDict, self.frozenOrderDict, self.frozenApplSeqNumDict, self.bid_price_levels, self.bid_order_nums, self.bidnq, self.bid_qty, self.bid_amount, \
                              self.ask_price_levels, self.ask_order_nums, self.asknq, self.ask_qty, self.ask_amount =\
            self.unfrozen(askOrderPrice, 2, self.askDict, self.frozenAskDict, self.frozenOrderDict, self.frozenApplSeqNumDict, self.bid_price_levels, self.bid_order_nums, self.bidnq, self.bid_qty, self.bid_amount, \
                              self.ask_price_levels, self.ask_order_nums, self.asknq, self.ask_qty, self.ask_amount)
            hasUnfrozen = 1
        ...
        if bidOrderQty == 0:
            self.orderDict.pop(BidApplSeqNum)
            self.bidnq.pop((bidOrderPrice, BidApplSeqNum))
            self.bid_order_nums -= 1
        else:
            self.orderDict[BidApplSeqNum] = (bidOrderPrice, bidOrderQty, bidOrderSide, bidOrderType, bidTag, 1)
            self.bidnq[bidOrderPrice, BidApplSeqNum] = np.int32(bidOrderQty)
        ...
        if askOrderQty == 0:
            self.orderDict.pop(OfferApplSeqNum)
            self.asknq.pop((askOrderPrice, OfferApplSeqNum))
            self.ask_order_nums -= 1           
        else:
            self.orderDict[OfferApplSeqNum] = (askOrderPrice, askOrderQty, askOrderSide, askOrderType, askTag, 1)
            self.asknq[askOrderPrice, OfferApplSeqNum] = np.int32(askOrderQty) 
    ...
    def removeOrderByCancel(self, caa, exchgTm, ApplSeqNum, cancelQty, BidApplSeqNum, OfferApplSeqNum):
        ...
        if cancelApplSeqNum in self.frozenOrderDict:
            ...
            if cancelApplSeqNum in [i[1] for i in self.asknq if i[0] == cancelPrice]:
                self.asknq[cancelPrice, cancelApplSeqNum] = np.int32(self.asknq[cancelPrice, cancelApplSeqNum] - cancelQty)
                if self.asknq[cancelPrice, cancelApplSeqNum] == 0:
                    self.asknq.pop((cancelPrice, cancelApplSeqNum))
            elif cancelApplSeqNum in [i[1] for i in self.bidnq if i[0] == cancelPrice]:
                self.bidnq[cancelPrice, cancelApplSeqNum] = np.int32(self.bidnq[cancelPrice, cancelApplSeqNum] - cancelQty)
                if self.bidnq[cancelPrice, cancelApplSeqNum] == 0:
                    self.bidnq.pop((cancelPrice, cancelApplSeqNum))
    ...
    def guessNBBO(self, caa, exchgTm, ApplSeqNum, aggSide, record=True):
        ...
        fakeBidnq = self.bidnq.copy()
        fakeAsknq = self.asknq.copy()
        ...
        hasTrade = 0
        fakeBidDict, fakeAskDict, fake_cum_volume, fake_cum_amount, fake_close, fakeBid1p, fakeAsk1p, fakeBidnq, fakeAsknq, fakeInsertion_bid, fakeInsertion_ask, fakeBid_qty, fakeAsk_qty, fakeBid_amount, \
        ...
        self.matchTrades(fakeBidDict, fakeAskDict, self.cum_volume, self.cum_amount, self.close, fakeBidnq, fakeAsknq, fakeInsertion_bid, fakeInsertion_ask, self.bid_qty, \
                         self.ask_qty, self.bid_amount, self.ask_amount, self.bid_price_levels, self.ask_price_levels, self.bid_order_nums, self.ask_order_nums, self.cur_cum_aggressive_limit_trade_on_1p_buy_qty, \
                         self.cur_cum_aggressive_limit_trade_on_1p_sell_qty, self.cur_cum_aggressive_limit_trade_on_1p_buy_amount, self.cur_cum_aggressive_limit_trade_on_1p_sell_amount, self.cur_cum_aggressive_limit_trade_over_1p_buy_qty, \
                         self.cur_cum_aggressive_limit_trade_over_1p_sell_qty, self.cur_cum_aggressive_limit_trade_over_1p_buy_amount, self.cur_cum_aggressive_limit_trade_over_1p_sell_amount, aggSide, Tag)
        ...
        if (self.skey >= 2300000) & (self.date >= 20200824):
            if aggSide == 1 and len(self.frozenBidDict) > 0:
                ...
                while len(self.frozenBidDict) > 0 and min(self.frozenBidDict.keys()) <= fakeMinBuyP:
                    ...
                    self.bidDict, self.frozenBidDict, self.frozenOrderDict, self.frozenApplSeqNumDict, self.bid_price_levels, self.bid_order_nums, self.bidnq, self.bid_qty, self.bid_amount, \
                                  self.ask_price_levels, self.ask_order_nums, self.asknq, self.ask_qty, self.ask_amount =\
                    self.unfrozen(fakeMinBuyP, 1, self.bidDict, self.frozenBidDict, self.frozenOrderDict, self.frozenApplSeqNumDict, self.bid_price_levels, self.bid_order_nums, self.bidnq, self.bid_qty, self.bid_amount, \
                                  self.ask_price_levels, self.ask_order_nums, self.asknq, self.ask_qty, self.ask_amount)
                    fakeBidDict, fakeFrozenBidDict, fakeFrozenOrderDict, fakeFrozenApplSeqNumDict, fakeBid_price_levels, fakeBid_order_nums, fakeBidnq, fakeBid_qty, fakeBid_amount, \
                                  fakeAsk_price_levels, fakeAsk_order_nums, fakeAsknq, fakeAsk_qty, fakeAsk_amount = \
                    self.unfrozen(fakeMinBuyP, 1, fakeBidDict, fakeFrozenBidDict, fakeFrozenOrderDict, fakeFrozenApplSeqNumDict, fakeBid_price_levels, fakeBid_order_nums, fakeBidnq, fakeBid_qty, fakeBid_amount, \
                                  fakeAsk_price_levels, fakeAsk_order_nums, fakeAsknq, fakeAsk_qty, fakeAsk_amount)
                    
                    fakeBidDict, fakeAskDict, fake_cum_volume, fake_cum_amount, fake_close, fakeBid1p, fakeAsk1p, fakeBidnq, fakeAsknq, fakeInsertion_bid, fakeInsertion_ask, fakeBid_qty, fakeAsk_qty, fakeBid_amount, \
                    fakeAsk_amount, fakeBid_price_levels, fakeAsk_price_levels, fakeBid_order_nums, fakeAsk_order_nums, fakeTrade_on_1p_buy_qty, fakeTrade_on_1p_sell_qty, fakeTrade_on_1p_buy_amount, fakeTrade_on_1p_sell_amount, \
                    fakeTrade_over_1p_buy_qty, fakeTrade_over_1p_sell_qty, fakeTrade_over_1p_buy_amount, fakeTrade_over_1p_sell_amount, fakeHasTrade = \
                    self.matchTrades(fakeBidDict, fakeAskDict, fake_cum_volume, fake_cum_amount, fake_close, fakeBidnq, fakeAsknq, fakeInsertion_bid, fakeInsertion_ask, fakeBid_qty, fakeAsk_qty, fakeBid_amount, \
                    fakeAsk_amount, fakeBid_price_levels, fakeAsk_price_levels, fakeBid_order_nums, fakeAsk_order_nums, fakeTrade_on_1p_buy_qty, fakeTrade_on_1p_sell_qty, fakeTrade_on_1p_buy_amount, fakeTrade_on_1p_sell_amount, \
                    fakeTrade_over_1p_buy_qty, fakeTrade_over_1p_sell_qty, fakeTrade_over_1p_buy_amount, fakeTrade_over_1p_sell_amount, aggSide, Tag)
            ...
            if aggSide == 2 and len(self.frozenAskDict) > 0:
                ...
                while len(self.frozenAskDict) > 0 and max(self.frozenAskDict.keys()) >= fakeMaxSellP:
                    ...
                    self.askDict, self.frozenAskDict, self.frozenOrderDict, self.frozenApplSeqNumDict, self.bid_price_levels, self.bid_order_nums, self.bidnq, self.bid_qty, self.bid_amount, \
                                  self.ask_price_levels, self.ask_order_nums, self.asknq, self.ask_qty, self.ask_amount =\
                    self.unfrozen(fakeMaxSellP, 2, self.askDict, self.frozenAskDict, self.frozenOrderDict, self.frozenApplSeqNumDict, self.bid_price_levels, self.bid_order_nums, self.bidnq, self.bid_qty, self.bid_amount, \
                                  self.ask_price_levels, self.ask_order_nums, self.asknq, self.ask_qty, self.ask_amount)
                    fakeAskDict, fakeFrozenAskDict, fakeFrozenOrderDict, fakeFrozenApplSeqNumDict, fakeBid_price_levels, fakeBid_order_nums, fakeBidnq, fakeBid_qty, fakeBid_amount, \
                                  fakeAsk_price_levels, fakeAsk_order_nums, fakeAsknq, fakeAsk_qty, fakeAsk_amount = \
                    self.unfrozen(fakeMaxSellP, 2, fakeAskDict, fakeFrozenAskDict, fakeFrozenOrderDict, fakeFrozenApplSeqNumDict, fakeBid_price_levels, fakeBid_order_nums, fakeBidnq, fakeBid_qty, fakeBid_amount, \
                                  fakeAsk_price_levels, fakeAsk_order_nums, fakeAsknq, fakeAsk_qty, fakeAsk_amount)
        ...
        if record:
            ...
            curBidN = [len([j[1] for j in fakeBidnq if j[0] == i]) for i in curBidP]
            ...
            curAskN = [len([j[1] for j in fakeAsknq if j[0] == i]) for i in curAskP]
    ...
    def matchTrades(self, bidDict, askDict, cum_volume, cum_amount, close, bidnq, asknq, insertion_bid, insertion_ask, bid_qty, ask_qty, bid_amount, ask_amount, bid_price_levels, ask_price_levels, \
        ...
        if bid1p >= ask1p:
            while bid1p >= ask1p:
                ...
                if tradeQty == bidQty:
                    ...
                    bidPLs = bidPLs[1:]
                    bid_price_levels -= 1
                    bid_order_nums -= len([i[1] for i in bidnq if i[0] == bid1p]) 
                else:
                    bidDict[bid1p] -= tradeQty
                    s_um = 0
                    pop_list = []
                    for seqNo in [i[1] for i in bidnq if i[0] == bid1p]:
                        s_um += bidnq[bid1p, seqNo]
                        if s_um > tradeQty:
                            bidnq[bid1p, seqNo] = np.int32(s_um - tradeQty)
                            break
                        elif s_um == tradeQty:
                ...
                if tradeQty == askQty:
                    ...
                    askPLs = askPLs[1:]
                    ask_price_levels -= 1
                    ask_order_nums -= len([i[1] for i in asknq if i[0] == ask1p])
                else:
                    askDict[ask1p] -= tradeQty
                    s_um = 0
                    pop_list = []
                    for seqNo in [i[1] for i in asknq if i[0] == ask1p]:
                        s_um += asknq[ask1p, seqNo]
                        if s_um > tradeQty:
                            asknq[ask1p, seqNo] = np.int32(s_um - tradeQty)
                            break
                        elif s_um == tradeQty:
                            pop_list.append(seqNo)
                            break
        ...
        return bidDict, askDict, cum_volume, cum_amount, close, bid1p, ask1p, bidnq, asknq, insertion_bid, insertion_ask, bid_qty, ask_qty, bid_amount, ask_amount, bid_price_levels, ask_price_levels, \
    bid_order_nums, ask_order_nums, cur_cum_aggressive_limit_trade_on_1p_buy_qty, cur_cum_aggressive_limit_trade_on_1p_sell_qty, cur_cum_aggressive_limit_trade_on_1p_buy_amount, \
    cur_cum_aggressive_limit_trade_on_1p_sell_amount, cur_cum_aggressive_limit_trade_over_1p_buy_qty, cur_cum_aggressive_limit_trade_over_1p_sell_qty, cur_cum_aggressive_limit_trade_over_1p_buy_amount, \
    cur_cum_aggressive_limit_trade_over_1p_sell_amount, hasTrade
                
    def unfrozen(self, price, side, orderbookDict, frozenOrderbookDict, frozenOrderDict, frozenApplSeqNumDict, bid_price_levels, bid_order_nums, bidnq, bid_qty, bid_amount, ask_price_levels, ask_order_nums,
                 asknq, ask_qty, ask_amount):
        ...
        for frozenP in frozenPLs:
            if side == 1:
                ...
                for frozenApplSeqNum in frozenApplSeqNumLs:
                    ...
                    bidnq[frozenP, frozenApplSeqNum] = new_qty
                    bid_qty += new_qty
            ...
            if side == 2:
                ...
                for frozenApplSeqNum in frozenApplSeqNumLs:
                    ask_order_nums += 1
                    new_qty = np.int32(frozenOrderDict[frozenApplSeqNum][1])
                    asknq[frozenP, frozenApplSeqNum] = new_qty
                    ask_qty += new_qty
        ...
        return orderbookDict, frozenOrderbookDict, frozenOrderDict, frozenApplSeqNumDict, bid_price_levels, bid_order_nums, bidnq, bid_qty, bid_amount, ask_price_levels, ask_order_nums, \
                 asknq, ask_qty, ask_amount
    ...
    def updateMktInfo(self, caa, exchgTm, ApplSeqNum, isImprove, record):
        ...
        if record:
            ...
            curBidN = [len([j[1] for j in self.bidnq if j[0] == i]) for i in curBidP]
            self.bidp += [curBidP + [0]*(self.outputLv-len(curBidP))]
            self.bidq += [curBidQ + [0]*(self.outputLv-len(curBidQ))]
            self.bidn += [curBidN + [0]*(self.outputLv - len(curBidN))]

            curAskQ = [self.askDict[i] for i in curAskP]
            curAskN = [len([j[1] for j in self.asknq if j[0] == i]) for i in curAskP]
            self.askp += [curAskP + [0]*(self.outputLv-len(curAskP))]
            self.askq += [curAskQ + [0]*(self.outputLv-len(curAskQ))]
            self.askn += [curAskN + [0]*(self.outputLv - len(curAskN))]
    ...
    def getSimMktInfo(self):
        ...
        bidInsert = pd.DataFrame(self.bid_insertion_qty, columns=['bid%sqInsert'%i for i in range(1, self.outputLv+1)])
        bidCancel = pd.DataFrame(self.bid_cancellation_qty, columns=['bid%sqCancel'%i for i in range(1, self.outputLv+1)])
        bidCancelSize = pd.DataFrame(self.bid_cancellation_amount, columns=['bid%ssCancel'%i for i in range(1, self.outputLv+1)])
        ...
        askInsert = pd.DataFrame(self.ask_insertion_qty, columns=['ask%sqInsert'%i for i in range(1, self.outputLv+1)])
        askCancel = pd.DataFrame(self.ask_cancellation_qty, columns=['ask%sqCancel'%i for i in range(1, self.outputLv+1)])
        askCancelSize = pd.DataFrame(self.ask_cancellation_amount, columns=['ask%ssCancel'%i for i in range(1, self.outputLv+1)])
        ...
        targetNLs = ['bid%sn'%i for i in range(self.outputLv, 0, -1)] + ['ask%sn'%i for i in range(1, self.outputLv+1)]
        targetInLs = ['bid%sqInsert'%i for i in range(self.outputLv, 0, -1)] + ['ask%sqInsert'%i for i in range(1, self.outputLv+1)]
        targetCqLs = ['bid%sqCancel'%i for i in range(self.outputLv, 0, -1)] + ['ask%sqCancel'%i for i in range(1, self.outputLv+1)]
        targetCaLs = ['bid%ssCancel'%i for i in range(self.outputLv, 0, -1)] + ['ask%ssCancel'%i for i in range(1, self.outputLv+1)]
        
        for col in ['cum_amount', 'close', 'total_bid_vwap', 'total_ask_vwap', 'cum_buy_market_order_amount', 'cum_sell_market_order_amount', \
                    'cum_buy_market_trade_amount', 'cum_sell_market_trade_amount', 'cum_buy_aggLimit_onNBBO_order_amount', 'cum_sell_aggLimit_onNBBO_order_amount', \
                    'cum_buy_aggLimit_onNBBO_trade_amount', 'cum_sell_aggLimit_onNBBO_trade_amount', 'cum_buy_aggLimit_improveNBBO_order_amount', 'cum_sell_aggLimit_improveNBBO_order_amount', \
                    'cum_buy_aggLimit_improveNBBO_trade_amount', 'cum_sell_aggLimit_improveNBBO_trade_amount'] + targetPLs + targetCaLs:
        ...
        targetCols = (['skey', 'date', 'time', 'caa', 'ApplSeqNum', 'cum_volume', 'cum_amount', 'close', 'openPrice', 'bboImprove'] + \
