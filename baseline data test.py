...
for y in ['20200817']:
    ...
    # readPath = 'A:\\KR_daily_data\\' + y + '\\SH\\snapshot\\Level1\\***'
    # dataPathLs = np.array(glob.glob(readPath))
    # dateLs = np.array([int(os.path.basename(i).split('.')[0]) for i in dataPathLs])
    # dataPathLs = dataPathLs[(dateLs >= 600000) & (dateLs <= 700000)]
    # logSH2 = []
    ...
    #         logSH2["ask" + str(i) + "p"] = logSH2["OfferPrice"].apply(lambda x: float(x.split(',')[0][1:]))
    #         logSH2["bid" + str(i) + "q"] = logSH2["BidOrderQty"].apply(lambda x: int(x.split(',')[0][1:]))
    #         logSH2["ask" + str(i) + "q"] = logSH2["OfferQty"].apply(lambda x: int(x.split(',')[0][1:]))
    #     elif i == 5:
    #         logSH2["bid" + str(i) + "p"] = logSH2["BidPrice"].apply(
    #             lambda x: float(x.split(',')[4][:-1]) if len(x.split(',')) == 5 else float(x.split(',')[4]))
    #         logSH2["ask" + str(i) + "p"] = logSH2["OfferPrice"].apply(
    #             lambda x: float(x.split(',')[4][:-1]) if len(x.split(',')) == 5 else float(x.split(',')[4]))
    #         logSH2["bid" + str(i) + "q"] = logSH2["BidOrderQty"].apply(
    #             lambda x: int(x.split(',')[4][:-1]) if len(x.split(',')) == 5 else int(x.split(',')[4]))
    #         logSH2["ask" + str(i) + "q"] = logSH2["OfferQty"].apply(
    #             lambda x: int(x.split(',')[4][:-1]) if len(x.split(',')) == 5 else int(x.split(',')[4]))
    #     else:
    #         logSH2["bid" + str(i) + "p"] = logSH2["BidPrice"].apply(lambda x: float(x.split(',')[i - 1]))
    #         logSH2["ask" + str(i) + "p"] = logSH2["OfferPrice"].apply(lambda x: float(x.split(',')[i - 1]))
    #         logSH2["bid" + str(i) + "q"] = logSH2["BidOrderQty"].apply(lambda x: int(x.split(',')[i - 1]))
    #         logSH2["ask" + str(i) + "q"] = logSH2["OfferQty"].apply(lambda x: int(x.split(',')[i - 1]))
    #
    # logSH2 = logSH2.rename(
    #     columns={"Volume": "cum_volume", "Amount": "cum_amount", "LastPx": "close", "OpenPx": "openPrice",
    #              "NumTrades": "numTrades"})
