...
class go():
    ...
    def run(self, s):
        ...
        try:
            ...
            data['ordering'] = data['ordering'].astype('int32')
            ...
            data = data[['skey', 'date', 'time', 'clockAtArrival', 'datetime', 'ordering', 'ApplSeqNum', 'bbo_improve', 'pass_filter', 'cum_volume', 'cum_amount',
                         ...
                         'bid2p', 'bid1p', 'ask1p', 'ask2p', 'ask3p', 'ask4p', 'ask5p', 'ask6p', 'ask7p', 'ask8p',
                         'ask9p', 'ask10p', 'bid10q', 'bid9q', 'bid8q', 'bid7q', 'bid6q', 'bid5q', 'bid4q', 'bid3q',
                         'bid2q', 'bid1q', 'ask1q', 'ask2q', 'ask3q', 'ask4q', 'ask5q', 'ask6q',
                         'ask7q', 'ask8q', 'ask9q', 'ask10q', 'bid10n',
...
class SimMktSnapshotAllNew():
        ...
        if orderType == '2':
            ...
            if side == 1:
                if len(self.bid) != 0:
                    ...
                    print('I dont understand this case!!!!!!!!!!!!!!!!!!!!!!!!!!!!')
                    self.tempOrder[applySeqNum] = (orderType, side, self.bid1p, qty, 0)
                    self.hasTempOrder = True
            else:
                if len(self.ask) != 0:
    ...
    def removeOrderByTrade(self, clockAtArrival, exchangeTime, seqNum, price, qty, bidApplSeqNum, offerApplSeqNum):
        ...
        # how can we make sure there must be one price level pop out? What if the quantity in the first price level is really high?
            ...
            if tempOrderType == '2':
                if cancelApplSeqNum in self.asknq[cancelPrice]:
                    ...
                    if self.bidnq[cancelPrice][cancelApplSeqNum] == 0:
                        ...
                        ##**##
            if tempStatus == 1:
                # also double check here, is the NBBO changed?
                self.updateMktInfo(clockAtArrival, exchangeTime, seqNum, 1, record=True)
            else:
                self.updateMktInfo(clockAtArrival, exchangeTime, seqNum, 1, record=False)
    ...
    def guessingTrade(self, clockAtArrival, exchangeTime, seqNum):
        ...
        self.exchangeTime.append(exchangeTime)
        self.sequenceNum.append(seqNum)
        # why? I think it should only happens when the first price level pop out
    ...
    def getAllInfo(self):
        ...
        targetCols = (['time', 'clockAtArrival', 'sequenceNo', 'StockID', 'cum_volume', 'cum_amount', 'open', 'close', 'bbo_improve'] +
        ...
        for col in ['StockID', 'total_bid_levels', 'total_ask_levels', 
                    'total_bid_orders', 'total_ask_orders', 'bbo_improve'] + bn_names + an_names + btopK_names + atopK_names:
...
if __name__ == '__main__':
    ...
    # start date
    thisDate = datetime.date(2020, 8, 20)
    while thisDate <= datetime.date(2020, 8, 20):
        ...
        mdOrderLog = db.read('md_order', start_date=thisDate_str, end_date=thisDate_str, symbol=[2000679])
        if mdOrderLog is None:
            thisDate = thisDate + datetime.timedelta(days=1)
            continue

        print(thisDate)
        mdTradeLog = db.read('md_trade', start_date=thisDate_str, end_date=thisDate_str, symbol=[2000679])
        mdLog = db.read('md_snapshot_l2', start_date=thisDate_str, end_date=thisDate_str, symbol=[2000679])

        orders_data = {}
        trades_data = {}
        prev_data = {}
        orders_data[2000679] = mdOrderLog
        trades_data[2000679] = mdTradeLog
        prev_data[2000679] = mdLog['prev_close'].iloc[0]
