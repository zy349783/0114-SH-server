...
funcPath = '/mnt/ShareWithServer/KR_upload_code'
sys.path.append(funcPath)
from generate5LvMBD_0111 import generateMBD
...
while thisDate <= datetime.date(2020, 12, 31):
    thisDate_str = str(thisDate).replace('-', '')
    db = DB("192.168.10.178", 'com_md_eq_cn', 'zhenyuy', 'bnONBrzSMGoE')

    mdOrderLog = db.read('md_order', start_date=thisDate_str, end_date=thisDate_str, symbol=[2000001])
    if mdOrderLog is None:
        thisDate = thisDate + datetime.timedelta(days=1)
        continue
    print(thisDate)
    sl = read_stock_daily(db1, 'mdbar1d_tr', start_date=int(thisDate_str), end_date=int(thisDate_str))
    sl = sl[(sl['skey'] > 2300000) & (sl['listed_days'] <= 5)]['skey'].unique()
    print(sl)
    if len(sl) == 0:
        thisDate = thisDate + datetime.timedelta(days=1)
        continue
    mdOrderLog = db.read('md_order', start_date=thisDate_str, end_date=thisDate_str, symbol=list(sl))
    ...
    pool = mp.Pool(processes=len(sl))
    stockDataLs = [stockData for _, stockData in msgData[msgData['skey'].isin(test_list)].groupby(['skey'])]
