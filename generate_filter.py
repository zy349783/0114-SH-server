...
# -*- coding: utf-8 -*-
"""
Created on Mon Sep 28 22:26:25 2020
...
#     sizeFilterData.to_pickle('/mnt/e/filter/' + str(stockID) + '.pkl')
#     print(str(stockID) + 'finished')
...
#     password = "bnONBrzSMGoE"
#     db = DB("192.168.10.178", database_name, user, password)
#     stockList = pd.read_csv('/mnt/ShareWithServer/AShare20160101_20200924.csv')
#     stockList['skey'] = np.where(stockList['StockID'].str[:2] == 'SH', stockList['StockID'].str[2:].astype(int) + 1000000, stockList['StockID'].str[2:].astype(int) + 2000000)
#     stockList = list(stockList['skey'].values)
#     pool = multiprocessing.Pool(60)    
#     pool.map(partial(sta_sizeFilter,startDate=startDate, endDate=endDate, 
...
def mp_size(startDate, endDate):
    ...
    stockList['skey'] = np.where(stockList['StockID'].str[:2] == 'SH', stockList['StockID'].str[2:].astype(int) + 1000000, stockList['StockID'].str[2:].astype(int) + 2000000)
    stockList = list(stockList['skey'].values)
    l1 = np.array(glob.glob('/mnt/e/filter/***'))
    l1 = np.array([int(os.path.basename(i).split('.')[0]) for i in l1])
    stockList = list(set(stockList) - set(l1))
    for i in stockList:
        print(i)
        try:
            ...
            print(e)
# sta_sizeFilter(2000001, 20170101, 20200924, regWindowSize = 20, weekInterval = 1)


mp_size(20170101, 20200924)