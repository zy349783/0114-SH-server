...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct 12 11:20:20 2020
...
class SimMktSnapshotAllNew():
    ...
    def getAllInfo(self):
        ...
        for col in ['cum_amount',
                    ...
                    'cum_buy_aggLimit_improveNBBO_trade_amount',
                    'cum_sell_aggLimit_improveNBBO_trade_amount'] + bsCancel_names + asCancel_names:
            mdData[col] = mdData[col].astype(float)
        mdData = mdData.round({'cum_amount':2, 'cum_buy_market_order_amount':2, 'cum_sell_market_order_amount':2, 'cum_buy_market_trade_amount':2, 'cum_sell_market_trade_amount':2,
                                'cum_buy_aggLimit_onNBBO_order_amount':2, 'cum_sell_aggLimit_onNBBO_order_amount':2, 'cum_buy_aggLimit_onNBBO_trade_amount':2, 'cum_sell_aggLimit_onNBBO_trade_amount':2,
                                'cum_buy_aggLimit_improveNBBO_order_amount':2, 'cum_sell_aggLimit_improveNBBO_order_amount':2, 'cum_buy_aggLimit_improveNBBO_trade_amount':2, 'cum_sell_aggLimit_improveNBBO_trade_amount':2,
                                'bid10sCancel':2, 'bid9sCancel':2, 'bid8sCancel':2, 'bid7sCancel':2, 'bid6sCancel':2, 'bid5sCancel':2, 'bid4sCancel':2, 'bid3sCancel':2, 'bid2sCancel':2, 'bid1sCancel':2,
                                'ask1sCancel':2, 'ask2sCancel':2, 'ask3sCancel':2, 'ask4sCancel':2, 'ask5sCancel':2, 'ask6sCancel':2, 'ask7sCancel':2, 'ask8sCancel':2, 'ask9sCancel':2, 'ask10sCancel':2})
        print(datetime.datetime.now() - startTm)       
...
if __name__ == '__main__':
    ...
    db = DB("192.168.10.178", 'com_md_eq_cn', 'zhenyuy', 'bnONBrzSMGoE')
    # start date
    thisDate = datetime.date(2020, 7, 14)
    while thisDate <= datetime.date(2020, 7, 15):
        intDate = (thisDate - datetime.date(1899, 12, 30)).days
        thisDate_str = str(thisDate).replace('-', '')
        ...
        re1 = re1.sort_values(by='count1', ascending=False)
        test_list = re1['skey'].values
        print(thisDate)
        mdOrderLog1 = mdOrderLog[mdOrderLog['skey'] == test_list[20]]
        del mdOrderLog
        mdTradeLog1 = db.read('md_trade', start_date=thisDate_str, end_date=thisDate_str, symbol=[test_list[20]])
        mdLog1 = db.read('md_snapshot_l2', start_date=thisDate_str, end_date=thisDate_str, symbol=[test_list[20]])
        mdLog1 = mdLog1.groupby('skey')['prev_close'].first().reset_index()['prev_close'].iloc[0]
        ...
        if 'pandas' in str(type(mdOrderLog1)):
            orders_data[test_list[20]] = mdOrderLog1
        if 'pandas' in str(type(mdTradeLog1)):
            trades_data[test_list[20]] = mdTradeLog1
        if 'pandas' in str(type(mdTradeLog1)):
            prev_data[test_list[20]] = mdLog1
            assert(mdLog1 > 0)
            del mdLog1
        g = go(thisDate_str, orders_data, trades_data, prev_data)
        del orders_data
