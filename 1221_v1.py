...
#     while thisDate <= datetime.date(2017, 6, 30):
...
if __name__ == '__main__':
    ...
    while thisDate <= datetime.date(2020, 4, 1):
        ...
        mdOrderLog = db.read('md_order', start_date=thisDate_str, end_date=thisDate_str, symbol=[2000911])
        if mdOrderLog is None:
            thisDate = thisDate + datetime.timedelta(days=1)
            continue 
        
        print(thisDate)
        mdTradeLog = db.read('md_trade', start_date=thisDate_str, end_date=thisDate_str, symbol=[2000911])
        mdLog = db.read('md_snapshot_l2', start_date=thisDate_str, end_date=thisDate_str, symbol=[2000911])
        mdLog = mdLog[(mdLog['open'] > 0)].groupby('skey')['prev_close', 'open'].first().reset_index()
        orders_data = {}
        trades_data = {}
        prev_data = {}
        ...
        if 'pandas' in str(type(mdOrderLog)):
            orders_data[2000911] = mdOrderLog
        if 'pandas' in str(type(mdTradeLog)):
            trades_data[2000911] = mdTradeLog
        if 'pandas' in str(type(mdTradeLog)):
            prev_data[2000911] = mdLog['prev_close'].iloc[0]
            open_data[2000911] = mdLog['open'].iloc[0]
        g = go(thisDate_str, orders_data, trades_data, prev_data, open_data)
        start = time.time()
        g.run(2000911)
        print(time.time() - start)
