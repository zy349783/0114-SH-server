    ...
    try:
        ...
        for rowEntry in stockDataNP:
            ...
            if rowEntry.isAuction:
                ...
                # if rowEntry.clockAtArrival > 1578015477260000:
                #     print(rowEntry)
        ...
        mdLog = db.read('md_snapshot_l2', start_date=str(data['date'].iloc[0]), end_date=str(data['date'].iloc[0]), symbol=data['skey'].iloc[0])     
        ...
        data = data[['skey', 'date', 'time', 'clockAtArrival', 'datetime', 'ordering', 'ApplSeqNum', 'bbo_improve', 'pass_filter', 'cum_trades_cnt', 'cum_volume', 'cum_amount',
        ...
        try:
            ...
            data.to_pickle('/mnt/ShareWithServer/2002192_1.pkl')
            del data
        except:
...
while thisDate <= datetime.date(2020, 1, 2):
    thisDate_str = str(thisDate).replace('-', '')
    db = DB("192.168.10.178", 'com_md_eq_cn', 'zhenyuy', 'bnONBrzSMGoE')

    mdOrderLog = db.read('md_order', start_date=thisDate_str, end_date=thisDate_str, symbol=[2002351])
    
    if mdOrderLog is None:
        thisDate = thisDate + datetime.timedelta(days=1)
        continue
    print(thisDate)
    mdTradeLog = db.read('md_trade', start_date=thisDate_str, end_date=thisDate_str, symbol=[2002351])
